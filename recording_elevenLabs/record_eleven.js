...
const OUT = '/tmp/pw-eleven/video';
const TIMELINE = '/tmp/pw-eleven/timeline.json';
const CLIPDIR = '/tmp/pw-eleven/clips';
const TTS = '/tmp/pw-eleven/eleven_tts.py';
const HOME = 'https://jerrywang951027.github.io/dummyFifa2026/';
const EMAIL = 'eternalwangjs@gmail.com';
const VOICE = process.env.ELEVEN_VOICE_ID || 'Rachel';
const RATE = 'eleven';
...
function pregenerate() {
  ...
  for (const [id, text] of Object.entries(NARR)) {
    const wav = `${CLIPDIR}/${id}.wav`;
    execSync(`python3 ${JSON.stringify(TTS)} ${JSON.stringify(text)} ${JSON.stringify(wav)}`, { stdio: ['ignore', 'ignore', 'inherit'] });
